...
const GlobeIcon = (props) => (
  <svg {...props} xmlns="[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 2a14.5 14.5 0 0 0 0 20 14.5 14.5 0 0 0 0-20"/><path d="M2 12h20"/></svg>
);
...
const NAV_LINKS = [
  { name: 'Official Website', href: '[https://bssc.live/](https://bssc.live/)', icon: GlobeIcon },
  { name: 'Explorer', href: '[https://explorer.bssc.live/](https://explorer.bssc.live/)', icon: SearchIcon },
  { name: 'BSSC Project GitHub', href: '[https://github.com/HaidarIDK/Binance-Super-Smart-Chain](https://github.com/HaidarIDK/Binance-Super-Smart-Chain)', icon: GitHubIcon },
  { name: 'X', href: '[https://x.com/bnbsolfork](https://x.com/bnbsolfork)', icon: XIcon },
];
...
export default function App() {
  ...
  // Function to call the local backend for AI analysis with explorer context
  const handleAsk = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(`Backend Request Failed: ${res.status} - ${errorData.error || res.statusText}`);
      }
      ...
      setResponse(`Error: ${err.message || 'Failed to process your request. Ensure the backend server is running.'}`);
    } finally {
      setLoading(false);
    }
  }, [input]);

  // Function to check the balance via the local backend's RPC proxy
  const handleCheckBalance = useCallback(async () => {
    ...
    try {
      ...
      // Call the local backend RPC proxy endpoint
      ...
      setBalance(`Error: Failed to fetch balance via proxy. ${err.message || 'Check the backend connection.'}`);
    } finally {
      setLoading(false);
    }
  }, [input]);
